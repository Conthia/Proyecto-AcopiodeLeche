...
| RF-28/RN-14 — Precio por temporada | 🟡 | Modelo y `obtenerPrecioVigenteEn()` completos, pero el % de reducción por sanción (RN-10, ligado a RN-14) es un valor **no confirmado con el cliente** y ya está codificado como si fuera definitivo (`CalculadoraLiquidacion.kt:18`) |
...
| ~~`data/sql/SqlEntregaRepository.kt:44-45` (`observarVolumenUltimaSemana`)~~ | ✅ **Resuelto** (rama `fix/grafico-volumen`): ahora calcula la suma real de `volumenLitros` agrupada por día vía la nueva query `sumaVolumenPorFecha` (SQLDelight), para la semana calendario actual (lunes-domingo), con `0.0` en los días sin entregas. `observarVolumenDeSemana(hoy: LocalDate = ...)` acepta la fecha como parámetro inyectable para tests. De paso se corrigió `VolumenSemanalChart.kt`, que resaltaba siempre el índice 4 (viernes) como "hoy" en vez del día real. Cubierto por 3 tests nuevos en `SqlEntregaRepositoryTest.kt` (caso "hoy = lunes", caso "hoy = domingo", y exclusión de la semana anterior). |
...
- Solo **3 de 17 repositorios SQL** tienen test contra base de datos real (`SqlEntregaRepositoryTest`, `SqlRutaRepositoryTest`, `SqlUsuarioRepositoryTest`, todos en `desktopTest`) — los otros 14 (Proveedor, CentroAcopio, Liquidacion, Notificacion, Pago, PrecioTemporada, ProduccionDerivado, Insumo, Sancion, CapacitacionCorrectiva, Asistencia, AnalisisCalidad, EquipoCampo, Reunion) solo se prueban indirectamente vía `Fake*Repository` (in-memory), que no valida el SQL real (mapeos de columnas, tipos, migraciones).
...
| UiState: rediseñar híbridos formulario+lista | rediseñar híbridos formulario+lista (AsignarRuta, SeleccionarProveedor, ColaEnvio, Reportes, LacteosHome, Liquidaciones): estado local en `MutableStateFlow` propios combinados con los flujos del repo para producir `UiState<Payload>`; los mutadores no tocan `_uiState`. `LiquidacionesViewModel` es además un caso más extremo: ni siquiera usa `combine()` de repositorios — `cargar()` es una carga imperativa "one-shot" invocada manualmente (al abrir la pantalla y desde `onGenerarClick`), más parecida a los formularios (`RegistrarEntregaViewModel` etc.) que al patrón reactivo combine→lista | `presentation/ruta/AsignarRutaViewModel.kt`, `presentation/entrega/SeleccionarProveedorViewModel.kt`, `presentation/cola/ColaEnvioViewModel.kt`, `presentation/reportes/ReportesViewModel.kt`, `presentation/dashboard/lacteos/LacteosHomeViewModel.kt`, `presentation/liquidacion/LiquidacionesViewModel.kt` | **Media** | Medio (un diseño por ViewModel, no es el patrón mecánico del piloto) |
