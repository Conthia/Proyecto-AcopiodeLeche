...
| `data/sql/SqlEntregaRepository.kt:44-45` (`observarVolumenUltimaSemana`) | **Devuelve una lista hardcodeada** `listOf(124.0, 168.0, 152.0, 191.0, 176.0, 204.0, 188.0)` en vez de calcular el volumen real de los últimos 7 días — el gráfico semanal del panel admin (`VolumenSemanalChart.kt`) muestra datos falsos siempre |
...
| Reportes / Panel Admin | Volumen semanal es un array hardcodeado, no refleja datos reales | `data/sql/SqlEntregaRepository.kt:44-45` | **Alta** | Bajo (una query SQL agregada por día) |
...
| Robustez UI | Ninguna pantalla de lista maneja error/carga; 0 `CircularProgressIndicator` | Todas las `*ViewModel.kt` de lista + sus `Screen.kt` | **Alta** | Medio (agregar `cargando`/`error` a cada `UiState` + UI) |
...
2. **Corregir el volumen semanal hardcodeado** en `SqlEntregaRepository.observarVolumenUltimaSemana()` — es una query SQL relativamente simple y hoy el panel de administración muestra datos falsos sin ningún indicio visual de que lo son.
