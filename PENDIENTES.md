...
| Robustez UI | 🟡 **En progreso, 4 lotes planeados en `feature/uistate-plantilla`** — `UiState<T>` genérico (Cargando/Éxito/Vacío/Error) implementado en `presentation/core/UiState.kt` y aplicado en `ProveedoresViewModel`/`ProveedoresScreen` (piloto). Lote 1: Centros/Reuniones/Asistencia. Lote 2: EntregasDelDia/AnálisisCalidadLista/ConciliaciónLista. Lote 3: SeleccionarProveedor/Notificaciones/ColaEnvio. Lote 4: Liquidaciones/Reportes/LacteosHome. 9 pantallas quedan fuera de este pendiente (ver fila siguiente) | Todas las `*ViewModel.kt` de lista + sus `Screen.kt`, salvo `proveedor/` | **Alta** | Medio (replicar el patrón ya validado, pantalla por pantalla) |
