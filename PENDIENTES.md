...
| Robustez UI | 🟡 **Piloto en Proveedores, falta replicar** — `UiState<T>` genérico (Cargando/Éxito/Vacío/Error) implementado en `presentation/core/UiState.kt` y aplicado en `ProveedoresViewModel`/`ProveedoresScreen` (rama `feature/uistate-plantilla`); las otras ~17 pantallas de lista siguen sin manejar error/carga | Todas las `*ViewModel.kt` de lista + sus `Screen.kt`, salvo `proveedor/` | **Alta** | Medio (replicar el patrón ya validado, pantalla por pantalla) |
