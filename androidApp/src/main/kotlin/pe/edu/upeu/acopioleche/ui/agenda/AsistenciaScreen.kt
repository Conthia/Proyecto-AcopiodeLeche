...
fun AsistenciaScreen(reunionId: String, alVolver: () -> Unit) {
    ...
    Scaffold(
        topBar = { AppTopBar(titulo = "Asistencia", subtitulo = uiState.temaReunion, alVolver = alVolver) },
        ...
        LazyColumn(
            ...
            item {
                AppCard {
                    Text(text = uiState.temaReunion, style = MaterialTheme.typography.titleMedium)
                    Text(text = uiState.fechaLugar, style = MaterialTheme.typography.bodyMedium)
                }
            }
            item {
                AppCard {
            ...
            item {
                Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                    ...
                    Text(
                        text = "${uiState.numeroPresentes} / ${uiState.convocados.size}",
                        color = VerdeOscuro,
                        style = MaterialTheme.typography.bodyLarge,
                    )
                }
            }
            items(uiState.convocados) { convocado ->
