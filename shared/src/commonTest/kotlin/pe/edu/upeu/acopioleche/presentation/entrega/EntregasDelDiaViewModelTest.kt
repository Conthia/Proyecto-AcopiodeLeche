...
class EntregasDelDiaViewModelTest {
    ...
    @Test
    fun `uiState es Exito con la lista de entregas cuando hay datos`() = runBlocking {
        val viewModel = EntregasDelDiaViewModel(
            scope = CoroutineScope(Dispatchers.Unconfined),
        ...
        assertIs<UiState.Exito<EntregasDelDiaUiState>>(estado)
        assertTrue(estado.datos.entregas.isNotEmpty())
    }

    @Test
    fun `uiState es Vacio cuando no hay entregas registradas hoy`() = runBlocking {
        val viewModel = EntregasDelDiaViewModel(
            scope = CoroutineScope(Dispatchers.Unconfined),
    ...
    private class EntregaRepositoryVacio(
        ...
        override fun observarPendientesDeSincronizar(): Flow<List<Entrega>> = MutableStateFlow(emptyList())
    }
