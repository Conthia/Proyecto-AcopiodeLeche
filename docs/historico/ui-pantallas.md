> **Nota histórica:** este documento corresponde al MVP inicial (rama `feature/pantallas-compose-mvp`,
> sesión 3 del curso, commit `c01ec5d`). Se conserva como referencia de cómo se diseñó ese
> prototipo, pero **no refleja el estado actual del proyecto** — pantallas, roles (p. ej. el rol
> "Técnico" ya no existe) y numeración de requerimientos (RF-xx) cambiaron en fases posteriores.
> Para el estado actual, ver `docs/modelo-dominio.md` y `docs/sincronizacion.md`.
